		...
		a link [
			if tag: match tag [
				...
				with tag [
					...
					if file? href [href: resolve href]
		...
		img image icon [
			either tag: match/report-to tag [
				...
				use [width height] with/only tag [
					...
					["<img" to-attr src to-attr width to-attr height to-attr alt { class="} either name = 'icon ["icon"]["img-responsive"] {" />}]
				]
			][
...
emit-flickr: func [spec [block!] /local photo id lookup][
	if verify [
		...
		switch type?/word photo/web_page [
			issue! [
				if parse/all photo/web_page amend [copy id some digit][
					photo/web_page: dirize join http://www.flickr.com/photos/anon/ photo/web_page
				]
			]
			url! [
				parse/all photo/web_page amend [
					"http://www.flickr.com/photos/"
					thru "/" copy id some digit opt [["/" | "?"] to end]
				]
			]
		][
 		...
 		lookup: any [
			load join wrt://space/flickr/p [id %.r]
			if attempt [
				require %markup/json.r
				lookup: load-json join http://www.flickr.com/services/oembed.json?
					to-webform join [url] photo/web_page
			][
				save/header join wrt://space/flickr/p [id %.r] body-of lookup context [
					Title: join "Flickr Lookup for #" id
					Date: now/date
				]
				lookup
			]
		...
		photo/title: rejoin ["&#8216;" sanitize photo/title "&#8217; by " sanitize photo/author_name]

		emit [
			{<div class="flickr img">^/<a href="} sanitize photo/web_page {">}
			{<img src="} sanitize photo/url {" width="} photo/width {" height="} photo/height
			{" alt="} photo/title {" class="img-responsive" /></a>^/</div>}
			either photo/titled [join "^/" [<figcaption> photo/title </figcaption>]][""]
		]
	]
]
...
normal: [
	...
	sect1: 
		(
			feed emit <section>
			feed emit <div class="page-header">
			feed emit [{<h1 id="} wordify form-para data {">}] emit-inline data emit </h1>
			feed emit </div>
		)
		in-sect1
		(feed emit </section>)
	sect2:
		(
			feed emit <section data-level="2">
			feed emit [{<h2 id="} wordify form-para data {">}] emit-inline data emit </h2>
		)
		in-sect2
		(feed emit </section>)
